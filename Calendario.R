Calendario<-function(frq,ma_af=4){
  ...
  if(assertthat::is.error(PP)){PP<-7}
  ...
  #Ordenar y aplicar otra vez
  clcntrs<-as.data.frame(kcl$centers)
  #Replace

  Clu_cont_n<-ifelse(kcl$cluster==order(clcntrs[[1]])[1],1,
                     ifelse(kcl$cluster==order(clcntrs[[1]])[2],2,3))
