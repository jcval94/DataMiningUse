SummaryFrecu<-function(data,Vars_clasif,Var_medida,anch=1000,reg_min=1){
  ...
  for (r in 1:length(unique(Var))){
    ...
    if(length(Var_r)<reg_min){
      c1<-rbind(c1,matrix(NA,1,19,dimnames = list(NA,c("min","q1","q2","q3","max","media","sd","Registros_Completos","vacios",
                                                       "ajuste","p_valor","Uni_min","Uni_q2","Uni_max","Uni_media",
                                                       "Uni_sd","vacios","Gto_Total","Unidades_Totales"))))
    }else{
      a<-try(Medida[Var %in% unique(Var_r)],silent = T)
      if(assertthat::is.error(a)){next()}
      if(length(a)==0){next()}
      #####################
      b<-estad(a,anch = anch)
      #####################Cambio por realizar
...
##SummaryFrecu(iris,"Species",names(iris)[1])
